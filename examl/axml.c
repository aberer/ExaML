...
int realMain(tree *tr, analdef *adef, int tid)
{
  ...
  _NON_LOCAL(tid, initializeTree(tr, adef)); 
  /* 
  ...
  */
  ...
  if(adef->useCheckpoint)
	...
	restart(tr); 
      ...
      computeBIGRAPID(tr, adef, TRUE, tid); 
    }
  else
    {
	...
	case TREE_EVALUATION: 
	  ...
	  modOpt(tr, adef->likelihoodEpsilon, tid);
	  if(tid == 0)
	    {
	      printLog(tr);
	...
	case BIG_RAPID_MODE:
	  ...
	  if(mpiState.numberOfThreads != 1)
	    {
	      printf("hybrid version not  implemented yet for tree inference.\n "); 
	      errorExit(-1, tid);
	    }
#endif
